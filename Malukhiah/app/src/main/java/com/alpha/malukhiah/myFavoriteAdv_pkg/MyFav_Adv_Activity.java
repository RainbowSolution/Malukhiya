...
public class MyFav_Adv_Activity extends AppCompatActivity implements View.OnClickListener {
    public List<Datum> mayfavList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userid = AppSession.getStringPreferences(MyFav_Adv_Activity.this, Constants.USER_ID);
        ...
        tv_title.setText(getResources().getString(R.string.myfavad));
        myfavorideAdv_rv = findViewById(R.id.id_rv_category);
        mayfavList = new ArrayList<>();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.id_notification:
                startActivity(new Intent(MyFav_Adv_Activity.this, Notification_Activity.class));
    ...
    public void getMyFavAdv() {
        if (Constant.isNetworkAvailable(getApplicationContext())) {
            CustomProgressbar.showProgressBar(MyFav_Adv_Activity.this, false);
            (RetrofitClient.getClient().getMyFavAdvList(userid)).enqueue(new Callback<MyFavAdsList>() {
                @Override
                public void onResponse(Call<MyFavAdsList> call, Response<MyFavAdsList> response) {
                    if (response.isSuccessful()) {
                        try {
                            CustomProgressbar.hideProgressBar();
                            Log.e("TAG", "State list response : " + new Gson().toJson(response.body()));
                            MyFavAdsList sectorPojo = response.body();
                            String resMessage = sectorPojo.getMessage();
                            if (sectorPojo.getStatus()) {
                                mayfavList = sectorPojo.getData();
                                MyFav_Adv_Adapter customAdapter = new MyFav_Adv_Adapter(MyFav_Adv_Activity.this, mayfavList);
                                myfavorideAdv_rv.setAdapter(customAdapter);
                            } else {
                                tv_data_not_id.setVisibility(View.VISIBLE);
                               // Toast.makeText(MyFav_Adv_Activity.this, resMessage, Toast.LENGTH_SHORT).show();
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                ...
                @Override
                public void onFailure(Call<MyFavAdsList> call, Throwable t) {
                    CustomProgressbar.hideProgressBar();
                    Toast.makeText(MyFav_Adv_Activity.this, getString(R.string.server_errors), Toast.LENGTH_SHORT).show();
                }
            });
        } else {
